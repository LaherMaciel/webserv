...
#include <fcntl.h> //to call fcntl() to make the fd a non-blocking fd.
...
int handle_client(Connection &client)
{
	...
	if (bytes_received > 0)
	{
		...
		std::cout << "Client " << client.fd << ": Received data:\n" << buffer << "\n";
	}
	else if (bytes_received == 0)
	{
		std::cout << "Client "<< client.fd << ": Closed connection" << std::endl;
		return (-1);
	}
	else
		std::cerr << "Failed to receive data from client (fd: " << client.fd << ")\n";
	if (bytes_received > 0)
	...
	while (true)
	{
		...
		for (size_t i = 0; i < tmp.size(); ++i)
		{
			...
			if (tmp[i].fd == server_fd)
			{
				...
				{
					std::cerr << "Filded to create a Non-Block fd" << std::endl;
					close(client_fd);
					continue ;
				}
